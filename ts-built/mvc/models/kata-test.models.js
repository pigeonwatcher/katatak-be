...
module.exports.insertSolutionToTests = (user_id, solutionToTest, kata_id, test_path) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    return new Promise((resolve, reject) => {
        ...
        (0, child_process_1.exec)(`npx jest ${test_path} ${id} "${solutionToTest}"`, (error, stdout, stderr) => {
            ...
            const usefulLogs = allLogs.slice(allLogs.indexOf("delete from here") + 1, allLogs.length);
            if (error) {
                // console.log(error, "<< error in model");
                // console.log(stdout, "<< stdout");
                // console.log(stderr, "<< stderr");
                const test_list = stderr.slice(stderr.indexOf(")") + 4, stderr.indexOf(" ●"));
                const success = false;
                //clearTimeout(timer);
                resolve({
                    success: false,
