...
(0, globals_1.describe)("/api/test/:kata_id", () => {
    ...
    globals_1.test.skip("POST: 408 responds with appropriate status code and error message if request times out (infinite loop)", () => __awaiter(void 0, void 0, void 0, function* () {
        const response = yield request(app)
            .post("/api/test/1")
            .send({
            user_id: 1,
            ...
            .expect(408);
        (0, globals_1.expect)(response.body.msg).toBe("408: Request timeout - check for an infinite loop..");
    }));
});
